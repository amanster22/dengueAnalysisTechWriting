...
```{r}
# --- RE-LOAD SPATIAL DATA WITH POPULATION ---
# We must re-create the spatial object using 'data_cleaned' because the 
# previous section (YoY) overwrote 'data_sf_wgs84' with data that lacked population.

# 1. Convert CLEANED data (which contains Population) to sf
data_sf_pop <- st_as_sf(data_cleaned, coords = c("X", "Y"), crs = 6371)
data_sf_pop_wgs84 <- st_transform(data_sf_pop, crs = 4326)

# 2. Load and transform the neighborhood shapefile (if not already loaded)
town_map_raw <- st_read("data/Merida_S25/Merida_Den12_13_14_15/Merida_Den12_13_14_15.shp")
town_map_wgs84 <- st_transform(town_map_raw, crs = 4326)
...
m_population <- leaflet(data_sf_pop_wgs84) %>%
  ...
  addPolygons(
    ...
    group = "Neighborhoods",
    fillOpacity = 0.1,
    color = "#ffffff", # White lines show up better on Satellite
    weight = 1,
    opacity = 0.5,
    popup = ~SP_ID
  ) %>%
...
# 4. Create Map
# NOTE: We pass 'data_sf_rates_wgs84' to leaflet(), not the generic 'data_sf_wgs84'
...
```{r}
# --- 1. DATA PREPARATION ---
# Ensure we have the correct spatial object with both columns
# (Assuming data_cleaned has 'Population', 'Rate2015', 'Den2015')
...
# Load Town Map (Neighborhoods)
town_map_raw <- st_read("data/Merida_S25/Merida_Den12_13_14_15/Merida_Den12_13_14_15.shp")
town_map_wgs84 <- st_transform(town_map_raw, crs = 4326)

# --- 2. DEFINE PALETTES ---
# Palette for Population (Blue/Green)
pal_pop <- colorNumeric(
  palette = "YlGnBu",
...
# Palette for Rate (Yellow/Orange/Red) - Represents "Heat"
...
# --- 3. DEFINE POPUPS ---
# We create one popup that shows EVERYTHING so it works for both layers
...
m_combined <- leaflet(data_sf_combined_wgs84) %>%
  ...
  # We make these slightly smaller (radius 5)
  addCircleMarkers(
    group = "Population",
  ...
  # D. Layer 2: 2015 Rate
  # We make these slightly larger (radius 8) or the same size depending on preference.
  # If they are larger, they form a "halo" around the population dot when both are on.
  addCircleMarkers(
    group = "2015 Rate",
    fillColor = ~pal_rate(Rate2015),
    fillOpacity = 0.6,    # Slightly more transparent to see overlap
    stroke = FALSE,       # No border for the rate to make it look more like a "glow"
    radius = 8,           # Larger radius so it's visible behind/around population
    popup = common_popup,
    label = ~paste0("Rate: ", round(Rate2015, 1))
  ) %>%
  
  # E. Legends (We add both!)
  addLegend(
    pal = pal_pop,
    values = ~Population,
    title = "Population",
    position = "bottomright",
    group = "Population"  # This binds the legend to the layer toggle!
  ) %>%
  ...
  addLegend(
    ...
    values = ~Rate2015,
    title = "Rate (per 10k)",
    position = "bottomleft", # Put on left to avoid crowding
    group = "2015 Rate"      # This binds the legend to the layer toggle!
  ) %>%
  
  # F. Layer Control ( The Toggles )
  addLayersControl(
    baseGroups = c("Satellite", "Light Map"),
  ...
  # G. Hide one layer by default? (Optional)
  # If you want them both visible at start, comment this out.
  # If you want to start with just Rate, hide Population:
