...
# 1. Prepare Spatial Data & Handle NAs
# Replace NA rates with 0 to prevent errors in localmoran.
...
# 3. Function to Calculate LISA and Classify Clusters
calculate_lisa <- function(data, variable_name, year_label, listw) {
  ...
  # Extract P-value by INDEX (Column 5) to avoid name mismatch errors
  p_value <- df_lisa[, 5]
  ...
  df_lisa$Cluster <- case_when(
    ...
    z_x > 0 & lag_x > 0 ~ "High-High", # Hotspot
    z_x < 0 & lag_x < 0 ~ "Low-Low",   # Coldspot
    z_x > 0 & lag_x < 0 ~ "High-Low",  # Spatial Outlier
    z_x < 0 & lag_x > 0 ~ "Low-High",  # Spatial Outlier
    TRUE ~ "Not Significant"
  )
  
  # Add coordinates and year
  df_lisa$X <- data$X
  df_lisa$Y <- data$Y
...
# 5. Visualization
lisa_colors <- c(
  "High-High" = "#FF0000",       # Red (Hotspot)
  "Low-Low" = "#0000FF",         # Blue (Coldspot)
  "High-Low" = "#FFC0CB",        # Pink (Outlier)
  "Low-High" = "#87CEEB",        # Light Blue (Outlier)
  "Not Significant" = "#eeeeee"  # Grey
)

ggplot(lisa_all, aes(x = X, y = Y, color = Cluster)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_manual(values = lisa_colors) +
  facet_wrap(~Year, ncol = 2) +
  labs(
    title = "LISA Cluster Map: Dengue Incidence Rates (2012-2015)",
    subtitle = "Local Indicators of Spatial Association (Moran's I)",
    x = "X Coordinate (UTM)",
    y = "Y Coordinate (UTM)",
    color = "Cluster Type"
  ) +
