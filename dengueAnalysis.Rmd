...
pop_data <- read.csv('merida_census_final.csv')
...
```
```{r}
htmlwidgets::saveWidget(fig, "dengue_animation.html", selfcontained = TRUE)
```
...
```
```{r}
htmlwidgets::saveWidget(m_cases, "dengue_InteractiveCaseCount.html", selfcontained = TRUE)
```
...
```
```{r}
htmlwidgets::saveWidget(m, "dengue_YoY_map.html", selfcontained = TRUE)
```
