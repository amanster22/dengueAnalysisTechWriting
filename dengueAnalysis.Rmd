...
Our group is interested in public health, epidemiology, and the positive effect that well-developed data analysis can have on urban welfare. In part, this interest derives from our experiences with the COVID-19 pandemic and its widespread reporting on its pathology, treatment, and mitigation. Dengue Fever's effects are similar to COVID-19's, and the disease similarly impacts communities worldwide. However, its relatively slower spread and its prevalence predominantly in tropical areas with underdeveloped healthcare infrastructure have led to severe neglect compared to other dangerous diseases. This study is primarily an exercise in geospatial and temporal analysis of a viral disease.
...
# Load Data
data <- read.csv('data/Merida_Den12_13_14_15.csv')
pop_data <- read.csv('data/merida_census_final.csv')
...
```{r}
# 1. Ensure the ID columns are characters to prevent scientific notation issues during joining
data <- data %>% mutate(CVEGEO = as.character(CVEGEO))
pop_data <- pop_data %>% mutate(AGEB_ID = as.character(AGEB_ID))

# 2. Join the datasets
# We keep all dengue records (left_join) and match CVEGEO to AGEB_ID
data_merged <- data %>%
  left_join(pop_data, by = c("CVEGEO" = "AGEB_ID"))

# 3. Calculate Incidence Rates (Cases per 10,000 people)
# We handle division by zero (if population is missing or 0)
data_cleaned <- data_merged %>%
  mutate(
    # Create Rate columns
    Rate2012 = (Den2012 / Population) * 10000,
    Rate2013 = (Den2013 / Population) * 10000,
    Rate2014 = (Den2014 / Population) * 10000,
    Rate2015 = (Den2015 / Population) * 10000
  ) %>%
  # Replace any Infs or NaNs (from 0 population) with NA
  mutate(across(starts_with("Rate"), ~ifelse(is.finite(.), ., NA))) %>%
  # Select only the columns we need for analysis
  select(X, Y, SP_ID, CVEGEO, Population, starts_with("Den"), starts_with("Rate"))

data_cleaned <- data_cleaned %>%
  filter(!SP_ID %in% c(392, 451))

# Check for nulls in the new rate columns
...
Before we further analyze dengue incidence over time, let's briefly look at some data for mosquito ovitraps. Ovitraps allow researchers to estimate the number of mosquitoes in an area by replicating breeding environments and measuring egg count. As dengue is a mosquito-borne illness, we should keep in mind the relative density of mosquito eggs as we look at disease hot spots later on. For the purposes of this analysis, we will not run statistical tests examining the exact association between estimated mosquito density and dengue incidence, as we intend to focus more on geospatial and temporal analytics.

```{r mapping_plot}

map <- st_read("data/Merida_S25/Merida_Den12_13_14_15/Merida_Den12_13_14_15.shp")
ovitraps <- st_read("data/Merida_S25/Ovitraps_Sum/Ovitraps_Sum.shp")
...
data_long <- data_cleaned %>%
  pivot_longer(
    # CHANGE HERE: explicitly select the 4 year columns OR use a regex pattern
    cols = c(Den2012, Den2013, Den2014, Den2015), 
    names_to = "Year",
    names_prefix = "Den",
    values_to = "Cases"
  ) %>%
...
fig <- plot_ly(
  ...
  animation_opts(
    frame = 5000,      # 5 seconds per frame
    transition = 1500,  # 0.8 second transition
    easing = "linear" 
  ) %>%
...
town_map_raw <- st_read("data/Merida_S25/Merida_Den12_13_14_15/Merida_Den12_13_14_15.shp")
...
### 8. Exploratory Analysis 6: Population Density vs. Incidence Rate

In the previous sections we looked at dengue fever's absolute case counts. However absolute case counts are often not a good indicator for hot spot as population is a major confounding variable in our analysis for hotspots because areas with higher population are much likely to have higher absolute number of people that have been infected. Thus to mitigate this issue of differing populations which in the neighborhoods of Merida we used web scraping on the following site: <https://www.citypopulation.de/en/mexico/agglomerida/> to obtain the population of each area within Merida. To isolate the "aggressiveness" of the virus from simple population density, we must analyze the Incidence Rate (cases per 10,000 residents).

#### a. Population visualization

```{r}

data_sf_pop <- st_as_sf(data_cleaned, coords = c("X", "Y"), crs = 6371)
data_sf_pop_wgs84 <- st_transform(data_sf_pop, crs = 4326)
...
# Create Palette for Population
pal_pop <- colorNumeric(
  palette = "YlGnBu",
  domain = data_sf_pop_wgs84$Population,
  na.color = "#a0a0a0"
)

# Create Popup for Population
popup_pop <- ~lapply(sprintf(
  "<strong>ID: %s</strong><br/>Population: %s residents", 
  SP_ID, comma(Population)
), HTML)

# Create the Map
m_population <- leaflet(data_sf_pop_wgs84) %>%
  # Base Groups (Satellite vs Light)
  addProviderTiles(providers$Esri.WorldImagery, group = "Satellite") %>%
  addProviderTiles(providers$CartoDB.Positron, group = "Light Map") %>%
  ...
  # Focus the map
  fitBounds(
    ~as.numeric(st_bbox(geometry)[1]), ~as.numeric(st_bbox(geometry)[2]),
    ~as.numeric(st_bbox(geometry)[3]), ~as.numeric(st_bbox(geometry)[4])
  ) %>%

  # Data Layer: Population
  addCircleMarkers(
    group = "Population",
    fillColor = ~pal_pop(Population), # Data color
    fillOpacity = 0.8,
    stroke = TRUE, 
    weight = 1, 
    ...
    radius = 6,
    popup = popup_pop,
    label = ~paste0("Pop: ", comma(Population))
  ) %>%
  ...
  # Layer Control
  addLayersControl(
    baseGroups = c("Satellite", "Light Map"),
    overlayGroups = c("Population", "Neighborhoods"),
    options = layersControlOptions(collapsed = FALSE)
  )
...
We first study the pure population count or denominator of our new rate that we are studying. We use a yellow and blue scale to study the population where yellow represents low population while blue represents higher with the scale from 0 to 5000. This visualization confirms our hypothesis that the population of the city is heterogeneous. If dengue fever is purely spread by human to human proximity we would expect the disease hotspots to perfectly mirror these dark blue high-density zones. Another thing to note is that the center of the city is not the area with the highest population in fact some parts in the central city have significantly lower population than the outskirts of the city.

#### b. 2015 Incidence Rate

```{r}
# --- 2015 RATE MAP ---

# 1. PREPARE DATA: Create a specific spatial object for Rates
data_sf_rates <- st_as_sf(data_cleaned, coords = c("X", "Y"), crs = 6371)
data_sf_rates_wgs84 <- st_transform(data_sf_rates, crs = 4326)

# 2. Create Palette
pal_rate_15 <- colorNumeric(
  palette = "YlOrRd",
  domain = data_sf_rates_wgs84$Rate2015,
  na.color = "#a0a0a0"
)

# 3. Create Popup
popup_rate_15 <- ~lapply(sprintf(
  "<strong>ID: %s</strong><br/>2015 Cases: %d<br/>Population: %s<br/><strong>Rate: %.2f per 10k</strong>", 
  SP_ID, Den2015, comma(Population), Rate2015
), HTML)

# 4. Create Map
m_rate_2015 <- leaflet(data_sf_rates_wgs84) %>%
  addProviderTiles(providers$Esri.WorldImagery, group = "Satellite") %>%
  addProviderTiles(providers$CartoDB.Positron, group = "Light Map") %>%
  ...
  addCircleMarkers(
    group = "2015 Rate",
    fillColor = ~pal_rate_15(Rate2015),
    fillOpacity = 0.9,
    stroke = TRUE, 
    weight = 1, 
    color = "black", 
    radius = 6,
    popup = popup_rate_15,
    label = ~paste0("Rate: ", round(Rate2015, 1))
  ) %>%
  
  addLegend(
    pal = pal_rate_15,
    values = ~Rate2015,
    title = "2015 Rate (per 10k)",
    position = "bottomright",
    opacity = 1
  ) %>%
  ...
  addLayersControl(
    baseGroups = c("Satellite", "Light Map"),
    overlayGroups = c("2015 Rate", "Neighborhoods"),
    options = layersControlOptions(collapsed = FALSE)
  )
...
Next we analyze the rate we proposed, where we calculate the predicted number of incidences in 10,000 people by the following formula: infected/population \* 10,000, for the year 2015. Using the yellow red scale where yellow means a lower rate while red means a higher rate we have the scale set from 0 to over 1000.

Unlike the absolute case map (Section 6), which highlights where the most sick people are, this map highlights where the highest probability of getting sick exists. We observe distinct clusters of high infection rates (dark red) that appear locally concentrated rather than randomly distributed across the city

#### c. Combined Analysis

```{r}
# --- 1. DATA PREPARATION ---
data_sf_combined <- st_as_sf(data_cleaned, coords = c("X", "Y"), crs = 6371)
data_sf_combined_wgs84 <- st_transform(data_sf_combined, crs = 4326)
...
pal_pop <- colorNumeric(
  palette = "YlGnBu",
  domain = data_sf_combined_wgs84$Population,
  na.color = "#a0a0a0"
)

pal_rate <- colorNumeric(
  palette = "YlOrRd",
  domain = data_sf_combined_wgs84$Rate2015,
  na.color = "#a0a0a0"
)

# --- 3. DEFINE POPUPS ---
common_popup <- ~lapply(sprintf(
  "<strong>ID: %s</strong><br/>
   Population: %s<br/>
   2015 Cases: %d<br/>
   <strong>Rate: %.2f per 10k</strong>", 
  SP_ID, comma(Population), Den2015, Rate2015
), HTML)

# --- 4. CREATE COMBINED MAP ---
m_combined <- leaflet(data_sf_combined_wgs84) %>%
  
  # A. Base Layers (The Backgrounds)
  addProviderTiles(providers$Esri.WorldImagery, group = "Satellite") %>%
  addProviderTiles(providers$CartoDB.Positron, group = "Light Map") %>%
  
  # B. Neighborhood Lines (Context)
  addPolygons(
    data = town_map_wgs84,
    group = "Neighborhoods",
  ...
  addLayersControl(
    baseGroups = c("Satellite", "Light Map"),
    overlayGroups = c("Population", "2015 Rate", "Neighborhoods"),
    options = layersControlOptions(collapsed = FALSE) 
  )
  
  # G. Hide one layer by default
...
This final map overlays both the population density and the 2015 incidence rate which allows us to direclty compare the two results using layer control.

This comparison reveals that Dengue incidences in Merida are not strictly correlated with population density. By toggling between the layers, we can identify areas with high infection rates (dark red on the Rate layer) that do not correspond to the highest population centers (dark blue on the Population layer). This disconnect suggests that environmental factors such as the vector breeding sites identified in the Ovitrap data likely play a more significant role in driving the 2015 resurgence than population density alone. The virus is more dependent on specific local conditions to spread efficiently resulting in high risk zones even in areas that are not the most densely populated.

### 9. Exploratory Analysis 7: LISA Analysis

```{r}

# Get rid of NA values
data_lisa_ready <- data_cleaned %>%
  mutate(across(starts_with("Rate"), ~replace_na(., 0)))

# Spatial Weighting
coords <- cbind(data_lisa_ready$X, data_lisa_ready$Y)
knn <- knearneigh(coords, k = 6)
nb <- knn2nb(knn)
listw <- nb2listw(nb, style = "W")

# LISA Calculation
calculate_lisa <- function(data, variable_name, year_label, listw) {
  x <- data[[variable_name]]
  local <- localmoran(x, listw, alternative = "two.sided")
  df_lisa <- as.data.frame(local)
  p_value <- df_lisa[, 5]
  z_x <- (x - mean(x)) / sd(x)
  lag_x <- lag.listw(listw, z_x)
  
  df_lisa$Cluster <- case_when(
    p_value > 0.05 ~ "Not Significant",
    z_x > 0 & lag_x > 0 ~ "High-High",
    z_x < 0 & lag_x < 0 ~ "Low-Low",
    z_x > 0 & lag_x < 0 ~ "High-Low",
    z_x < 0 & lag_x > 0 ~ "Low-High",
    TRUE ~ "Not Significant"
  )
  
  # Colors
  df_lisa$Color <- case_when(
    df_lisa$Cluster == "High-High" ~ "red",
    df_lisa$Cluster == "Low-High"  ~ "blue",
    df_lisa$Cluster == "Not Significant" ~ "grey",
    TRUE ~ "grey"
  )
  
  df_lisa$SP_ID <- data$SP_ID
  df_lisa$Rate <- round(x, 2)
  df_lisa$X <- data$X
  df_lisa$Y <- data$Y
  df_lisa$Year <- year_label
  
  return(df_lisa)
}

# Run for all years
lisa_2012 <- calculate_lisa(data_lisa_ready, "Rate2012", "2012", listw)
lisa_2013 <- calculate_lisa(data_lisa_ready, "Rate2013", "2013", listw)
lisa_2014 <- calculate_lisa(data_lisa_ready, "Rate2014", "2014", listw)
lisa_2015 <- calculate_lisa(data_lisa_ready, "Rate2015", "2015", listw)

# Combine years
lisa_all <- bind_rows(lisa_2012, lisa_2013, lisa_2014, lisa_2015)


lisa_sf <- st_as_sf(lisa_all, coords = c("X", "Y"), crs = 6371)
lisa_sf_wgs84 <- st_transform(lisa_sf, crs = 4326)

lisa_12_map <- filter(lisa_sf_wgs84, Year == "2012")
lisa_13_map <- filter(lisa_sf_wgs84, Year == "2013")
lisa_14_map <- filter(lisa_sf_wgs84, Year == "2014")
lisa_15_map <- filter(lisa_sf_wgs84, Year == "2015")

make_popup <- function(id, rate, cluster, year) {
  sprintf(
    "<strong>Year: %s</strong><br/>ID: %s<br/>Rate: %s<br/>Cluster: %s",
    year, id, rate, cluster
  ) 
}
...
leaflet() %>%
  ...
  addCircleMarkers(
    ...
    group = "2012 LISA",
    radius = 5,
    fillColor = ~Color,   # USE THE EXPLICIT COLOR COLUMN
    fillOpacity = 0.9,
    stroke = FALSE,
    popup = ~make_popup(SP_ID, Rate, Cluster, Year)
  ) %>%
  ...
  addLegend(
    ...
    labels = c("High-High", "Low-High", "Not Significant"),
    title = "LISA Cluster",
    opacity = 1
  ) %>%
  ...
  # Year Layers
  addLayersControl(
    overlayGroups = c("2012 LISA", "2013 LISA", "2014 LISA", "2015 LISA"),
    options = layersControlOptions(collapsed = FALSE)
  )
```

This final analysis utilizes local indicator of spatial association (LISA). High-high clusters represent areas with high dengue incidence surrounded by other areas of high incidence. This means that they are particular centers of epidemiological interest. Conversely, low-high clusters represent areas of low incidence surrounded by areas of relatively high incidence. These areas are also important to study because their relative resistance to disease spread could lend important insight into disease mitigation strategy that can be applied to other areas. We can see that high-high clusters remain in the center of the city and progressively expand throughout the years. New hotspots also emerge in the southwest of the city and clusters disappear from the northeast.
...
Our LISA analysis was significant because it revealed that Dengue fever in Mérida maintained a persistent central reservoir even during 2013 and 2014 when aggregate cases fell considerably. While city-wide totals suggested the outbreak was contained, the spatial analysis exposed a stable "high-high" cluster that likely acted as the stronghold from which the massive 2015 resurgence may have originated. Furthermore, the analysis identified specific relatively safe neighborhoods (low-high outliers) that remained resistant despite being surrounded by relatively high rates of infection, showing locations for researchers to investigate effective protective factor that could be applied elsewhere.
